...
var JUnitReporter = function (baseReporterDecorator, config, logger, helper, formatError) {
  ...
  var writeXmlForBrowser = function (browserId, browserName) {
    var safeBrowserName = browserName.replace(/ /g, '_')
    var newOutputFile
    ...
    var xmlToOutput = suites[browserId]
    if (!xmlToOutput) {
  ...
  this.onBrowserComplete = function (browser) {
    ...
    var stats = allBrowsersStats[browser.id]
    if (!stats) {
      allBrowsersStats[browser.id] = stats = {};
    }
    stats.tests = (stats.tests || 0) + (result.total ? result.total : 0);
    stats.errors = (stats.errors || 0) + (result.disconnected || result.error ? 1 : 0);
    stats.failures: (stats.failures || 0) + (result.failed ? result.failed : 0);
    stats.time: Math.max((stats.time || 0), (result.netTime || 0) / 1000);
    stats.name = browser.name
  }
  ...
  this.onRunComplete = function () {
    for (var browser in allBrowsers) {
      ...
      var browserStats = allBrowsersStats[browser.id];
      if (!suite || !browserStats) {
        continue // don't die if browser didn't start
      }

      suite.att('tests', browserStats[tests])
      suite.att('errors', browserStats[errors])
      suite.att('failures', browserStats[failures])
      suite.att('time', browserStats[time])

      allMessages.push("browser.id=" + browser.id);

      suite.ele('system-out').dat(allMessages.join() + '\n')
      suite.ele('system-err')
    }

    for (var browserId in allBrowsersStats) {
      writeXmlForBrowser(browserId, allBrowsersStats[browserId].name)
      // Release memory held by the test suite.
      suites[browserId] = null
    }
