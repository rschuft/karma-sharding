...
//   index collectors by browser.name instead of browser.id
...
var globalSourceCache = require('../node_modules/karma-coverage/lib/source-cache')
var coverageMap = require('../node_modules/karma-coverage/lib/coverage-map')
var SourceCacheStore = require('../node_modules/karma-coverage/lib/source-cache-store')
...
var CoverageReporter = function (rootConfig, helper, logger, emitter) {
  var log = logger.create('coverage:karma-sharding')
  ...
  this.onBrowserStart = function (browser) {
    if (!collectors[browser.name]) {
      collectors[browser.name] = new istanbul.Collector()

      if (!includeAllSources) return

      collectors[browser.name].add(coverageMap.get())
    }
  }

  this.onBrowserComplete = function (browser, result) {
    var collector = collectors[browser.name]
  ...
  this.onSpecComplete = function (browser, result) {
    ...
    collectors[browser.name].add(result.coverage)
  }
  ...
  this.onRunComplete = function (browsers, results) {
    ...
    reporters.forEach(function (reporterConfig) {
      browsers.forEach(function (browser) {
        ...
        if (config.hasOwnProperty('check') && !checkedCoverage[browser.name]) {
          checkedCoverage[browser.name] = true
